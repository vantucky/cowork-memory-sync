...
The current space's memory directory is in your system prompt under the auto-memory section. It looks like:
...
Every snapshot records **who** wrote it (`user`) and on **which machine** (`machine`). Both are stored once per Mac at `~/.config/cowork-memory-sync/identity.json`:

```json
{ "user": "richard.dyer", "machine": "workhorse" }
```

If Cowork's sandbox can't reach `~/.config/`, the values fall back to per-space fields in `.sync-link.json` — same effect, just set per-link.

### Probe global identity

Try to **Read** `$HOME/.config/cowork-memory-sync/identity.json` (use the Read tool, not Bash — Bash in Cowork's sandbox returns a misleading `Permission denied on /Users` for paths outside connected folders; Read returns a clean "outside connected folders" error).
...
- **Read fails, "outside this session's connected folders"** → sandbox barrier. Call `mcp__cowork__request_cowork_directory` for `~/.config/cowork-memory-sync/` (tell the user: *"One-time grant so I can store your sync identity — your name + this Mac's name — shared across all linked spaces."*). Retry the Read once. If it still fails, set `USE_PER_SPACE_IDENTITY = true` and gather identity per-space in the flow below.

### Migrate legacy machine.txt (v2 → v3)

If `identity.json` didn't exist, also try to Read `$HOME/.config/cowork-memory-sync/machine.txt` (the v2 machine-name file). If it exists, use its contents as the default `IDENTITY_MACHINE`. (v3 supersedes `machine.txt` with `identity.json`; once you write `identity.json`, `machine.txt` is ignored.)
...
If `IDENTITY_MACHINE` is empty, run `hostname -s` via Bash as `DEFAULT_HOST`, then AskUserQuestion:
- **Question**: "What should this Mac be called in snapshots? System hostname is `<DEFAULT_HOST>`."
- **Header**: "Machine"
...
```bash
mkdir -p "$HOME/.config/cowork-memory-sync"
```

Then **Write** `$HOME/.config/cowork-memory-sync/identity.json` with `{ "user": "<IDENTITY_USER>", "machine": "<IDENTITY_MACHINE>" }` (2-space indent). If `USE_PER_SPACE_IDENTITY` is true, skip the global write — the values get stored in `.sync-link.json` when the link is written.
...
```bash
find ~/Library/CloudStorage -maxdepth 6 -type d -name _cowork-snapshots 2>/dev/null
```
...
- **If none, or "Create a new one"**: list candidate parent folders —
  ```bash
  find ~/Library/CloudStorage -maxdepth 3 -mindepth 2 -type d 2>/dev/null | grep -v "/_" | sort
  ```
  Present them (label = path relative to `~/Library/CloudStorage/`) plus `Type a custom path` and `Cancel`. For a custom path, validate `[[ -d "<path>" ]]`. Set `PARENT = <chosen>`, `STORE_PATH = $PARENT/_cowork-snapshots`, derive `ALIAS` from `PARENT`'s basename. Then `mkdir -p "$STORE_PATH"`.
...
3. **Copy your own snapshots** from the old `store_path` to `STORE_PATH_NEW`. Your files are those whose name matches `<date>-<IDENTITY_USER>-*.md`. List them:
   ```bash
   ls "<old_store_path>"/*-<IDENTITY_USER>-*.md 2>/dev/null
   ```
   Copy each with `cp` (do **not** move yet — copy first, verify, so a failure can't lose data). If the space had legacy v2 files (`<date>-<slug>.md`, no user segment) and history indicates they're yours (solo origin), copy those too.
...
- **Don't pick a folder outside `~/Library/CloudStorage/` without warning** — it won't sync, and for shared mode it won't reach other people at all.
